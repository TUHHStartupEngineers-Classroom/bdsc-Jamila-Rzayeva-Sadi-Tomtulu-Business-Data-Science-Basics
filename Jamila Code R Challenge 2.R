...
ID <- GET(glue("https://www.themoviedb.org/person/1245-scarlett-johansson")) %>% 
  .$content %>% 
  rawToChar() %>% 
  fromJSON() %>% 
  as.data.frame
...
db <- GET(glue("https://www.themoviedb.org/movie/240832-lucy")) %>% 
  .$content %>% 
  rawToChar() %>% 
  fromJSON() %>%
  as.data.frame %>%
...
db %>% ggplot(aes(x=reorder(title, as.numeric(year)), y=popularity)) +
  ...
  geom_col(fill = wes_palette("BottleRocket1", 2)[2]) +
